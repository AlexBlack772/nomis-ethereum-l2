// ------------------------------------------------------------------------------------------------------
// <copyright file="OptimismService.cs" company="Nomis">
...
using Nomis.Optimism.Calculators;
...
{
    /// <inheritdoc cref="IOptimismScoringService"/>
    internal sealed class OptimismService :
        BlockchainDescriptor,
        IOptimismScoringService,
        IHasDefiLlamaIntegration,
        ITransientService
    {
        private readonly IOptimismClient _client;
        ...
        /// <summary>
        /// Initialize <see cref="OptimismService"/>.
        /// </summary>
        /// <param name="settings"><see cref="OptimismSettings"/>.</param>
        /// <param name="client"><see cref="IOptimismClient"/>.</param>
        ...
        public OptimismService(
            IOptions<OptimismSettings> settings,
            IOptimismClient client,
        ...
        /// <inheritdoc />
        public string DefiLLamaChainId => "optimism";
            ...
            where TTransactionIntervalData : class, ITransactionIntervalData
        {
            ...
            var transactions = (await _client.GetTransactionsAsync<OptimismAccountNormalTransactions, OptimismAccountNormalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var internalTransactions = (await _client.GetTransactionsAsync<OptimismAccountInternalTransactions, OptimismAccountInternalTransaction>(request.Address).ConfigureAwait(false)).ToList();
            var erc20Tokens = (await _client.GetTransactionsAsync<OptimismAccountERC20TokenEvents, OptimismAccountERC20TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            var tokens = (await _client.GetTransactionsAsync<OptimismAccountERC721TokenEvents, OptimismAccountERC721TokenEvent>(request.Address).ConfigureAwait(false)).ToList();
            ...
            {
                aaveAccountDataResponse = (await _aaveService.GetAaveUserAccountDataAsync(AaveChain.Optimism, request.Address).ConfigureAwait(false)).Data;
            }

            #endregion Aave protocol

            var walletStats = new OptimismStatCalculator(
                    request.Address,
